...
function wordsMatch(scriptWord, asrWord) {
  ...
  // Levenshtein distance ≤ 1 (handles substitution, insertion, deletion correctly)
  if (Math.abs(a.length - b.length) <= 2 && a.length >= 4 && b.length >= 4) {
    // Full DP for small strings — O(n²) but n≤30 so irrelevant
      ...
      if (wordsMatch(scriptW, asrW)) {
        ...
        for (let skip = 1; skip <= 3 && localAsrIdx + skip < maxAsrConsume; skip++) {
          ...
          if (candidateIdx < asrWords.length && wordsMatch(scriptW, asrWords[candidateIdx].word)) {
            localAsrIdx += skip;
            if (firstMatchedAsrIdx === -1) firstMatchedAsrIdx = candidateIdx;
            lastMatchedAsrIdx = candidateIdx;
            matchedCount++;
            scriptIdx++;
            localAsrIdx++;
            asrSkipFound = true;
