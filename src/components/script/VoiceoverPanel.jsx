...
import { Loader2, Mic, Play, Pause, Download, Volume2, Square, Search, X, Zap, Globe, Sparkles } from 'lucide-react';

// ══════════════════════════════════════════════════════════════════
// Shared VoicePanel — used by MiniMax Direct and AI33 Pro
// ══════════════════════════════════════════════════════════════════
...
// ══════════════════════════════════════════════════════════════════
// Inworld AI Panel — different generate flow (start → process loop)
// ══════════════════════════════════════════════════════════════════
function InworldPanel({ project, script, onUpdate, settings, setSettings }) {
  ...
  useEffect(() => {
    ...
    base44.functions.invoke('listVoicesByProvider', { source: 'inworld' })
      .then(res => setVoices(res.data?.voices || []))
      .catch(err => { console.warn('Inworld voices failed:', err.message); setError('Inworld voices unavailable.'); })
  ...
  const handleGenerate = async () => {
    if (!script?.id || !selectedVoice) return;
    setGenerating(true); setError(''); setChunkProgress(null);

    // Step 1: Start — chunk script
    let startRes;
    try {
      startRes = await base44.functions.invoke('inworldVoiceover', {
        project_id: project.id, voice_id: selectedVoice, action: 'start',
      });
    } catch (err) { setError(err?.response?.data?.error || err.message); setGenerating(false); return; }

    if (startRes?.data?.error) { setError(startRes.data.error); setGenerating(false); return; }

    const totalChunks = startRes.data?.chunks_total || 0;
    setChunkProgress({ completed: 0, total: totalChunks, percent: 0 });

    // Step 2: Process loop — process chunks in batches
    const processInterval = setInterval(async () => {
      try {
        const res = await base44.functions.invoke('inworldVoiceover', {
          project_id: project.id, action: 'process',
        });
        const data = res.data;

        setChunkProgress({
          completed: data.chunks_completed || 0,
          total: data.chunks_total || totalChunks,
        ...
        if (data?.status === 'ready' && data?.voiceover_url) {
          const sr = await base44.entities.ProductionSettings.filter({ project_id: project.id });
          if (sr[0]) setSettings({ ...sr[0], voiceover_status: 'completed', voiceover_url: data.voiceover_url });
          setGenerating(false); setChunkProgress(null); clearInterval(processInterval); onUpdate?.();
        } else if (data?.status === 'failed') {
          setError(data.error || 'Inworld generation failed.'); setGenerating(false); setChunkProgress(null); clearInterval(processInterval);
        }
      } catch (e) { console.warn('Inworld process error:', e.message); }
    }, 5000);

    setTimeout(() => { clearInterval(processInterval); setGenerating(p => { if (p) setError('Still generating. Refresh to check.'); return false; }); }, 3600000);
  };
    ...
    <Card className="border-emerald-200">
      <CardHeader className="pb-3">
        <CardTitle className="text-base flex items-center gap-2">
          ...
          <Badge className="bg-emerald-100 text-emerald-700 text-[10px]">TTS 1.5</Badge>
          {settings?.voiceover_status === 'completed' && settings?.voiceover_url && <Badge className="bg-green-100 text-green-800 text-xs ml-auto">Ready</Badge>}
        </CardTitle>
      </CardHeader>
      <CardContent className="space-y-3">
          ...
          <>
            {selectedVoiceData && (
              <div className="flex items-center gap-2 p-2 rounded-lg border border-emerald-200 bg-emerald-50">
                <div className="flex-shrink-0 w-7 h-7 rounded-full bg-emerald-200 flex items-center justify-center"><Mic className="w-3 h-3 text-emerald-700" /></div>
                <div className="flex-1 min-w-0">
                  <p className="font-medium text-sm truncate">{selectedVoiceData.name}</p>
                  <p className="text-xs text-gray-500 truncate">{selectedVoiceData.description || [selectedVoiceData.labels?.gender, selectedVoiceData.labels?.use_case].filter(Boolean).join(' · ')}</p>
                </div>
                {selectedVoiceData.category === 'cloned' && <Badge className="bg-amber-100 text-amber-700 text-[9px]">Clone</Badge>}
              </div>
            )}

            <div className="space-y-2">
              <div className="relative"><Search className="w-3.5 h-3.5 absolute left-2 top-1/2 -translate-y-1/2 text-gray-400" /><Input placeholder="Search Inworld voices..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="pl-7 h-8 text-xs" />{searchQuery && <button onClick={() => setSearchQuery('')} className="absolute right-2 top-1/2 -translate-y-1/2 text-gray-400"><X className="w-3 h-3" /></button>}</div>
              <Select value={genderFilter} onValueChange={setGenderFilter}><SelectTrigger className="h-8 text-xs"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="all">All Genders</SelectItem><SelectItem value="male">Male</SelectItem><SelectItem value="female">Female</SelectItem></SelectContent></Select>
            </div>
            ...
            <div className="max-h-60 overflow-y-auto space-y-1 pr-1">
              {filteredVoices.map(v => (
                <div key={v.voice_id} onClick={() => setSelectedVoice(v.voice_id)}
                  className={`flex items-center gap-2 p-2 rounded-lg border cursor-pointer text-sm ${selectedVoice === v.voice_id ? 'bg-emerald-50 border-emerald-300' : 'bg-white hover:bg-gray-50 border-gray-200'}`}>
                  <div className="flex-shrink-0 w-6 h-6 rounded-full bg-gray-100 flex items-center justify-center"><Mic className="w-2.5 h-2.5 text-gray-400" /></div>
                  <div className="flex-1 min-w-0">
                    <p className="font-medium truncate text-xs">{v.name}</p>
                    <p className="text-[11px] text-gray-500 truncate">{v.description || [v.labels?.gender, v.labels?.use_case].filter(Boolean).join(' · ')}</p>
                  </div>
                  {v.category === 'cloned' && <Badge className="bg-amber-100 text-amber-700 text-[9px]">Clone</Badge>}
                  {selectedVoice === v.voice_id && <Badge className="bg-emerald-100 text-emerald-700 text-[9px]">✓</Badge>}
                </div>
              ))}
              {filteredVoices.length === 0 && <p className="text-xs text-gray-400 text-center py-3">No voices found</p>}
            </div>
        ...
        <Button onClick={handleGenerate} disabled={generating || !selectedVoice || !script} className="w-full bg-emerald-600 hover:bg-emerald-700">
          {generating ? <><Loader2 className="w-4 h-4 mr-2 animate-spin" /> Generating...</> : <><Sparkles className="w-4 h-4 mr-2" /> Generate with Inworld</>}
        </Button>

        {/* Chunk progress */}
        {generating && chunkProgress && chunkProgress.total > 0 && (
          <div className="bg-emerald-50 border border-emerald-200 rounded-lg p-3 space-y-2">
            <div className="flex items-center justify-between text-xs">
              <span className="font-medium text-emerald-700">Processing {chunkProgress.total} chunks...</span>
              <span className="text-emerald-600 font-mono">{chunkProgress.completed}/{chunkProgress.total}</span>
            </div>
            <div className="w-full bg-emerald-100 rounded-full h-2">
              <div className="bg-emerald-500 h-2 rounded-full transition-all duration-700" style={{ width: `${chunkProgress.percent || 0}%` }} />
            </div>
            {chunkProgress.failed > 0 && <p className="text-[11px] text-red-500">{chunkProgress.failed} chunks failed</p>}
          </div>
        )}
        ...
        {settings?.voiceover_url && settings.voiceover_status === 'completed' && (
          <div className="bg-gray-50 p-3 rounded-lg">
            ...
            <div className="flex items-center gap-2">
              <Button variant="outline" size="icon" className="h-8 w-8" onClick={togglePlay}>{playing ? <Pause className="w-3.5 h-3.5" /> : <Play className="w-3.5 h-3.5" />}</Button>
              <div className="flex-1"><p className="text-xs font-medium">Inworld Voiceover</p><p className="text-[11px] text-gray-500">Ready to play</p></div>
              <Button variant="outline" size="sm" className="h-7 text-xs" onClick={() => { const a = document.createElement('a'); a.href = settings.voiceover_url; a.download = `${project.name || 'voiceover'}_inworld.wav`; a.click(); }}><Download className="w-3 h-3 mr-1" /> Download</Button>
            </div>
          </div>
        )}
      </CardContent>
...
// ══════════════════════════════════════════════════════════════════
// Main VoiceoverPanel — renders all 3 panels
// ══════════════════════════════════════════════════════════════════
