          ...
          {sorted.map((script, idx) => (
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="font-medium">{idx === 0 ? 'Latest' : `Version ${sorted.length - idx}`}</p>
                  <p className="text-xs text-gray-500">
                    {format(new Date(script.created_date), 'MMM d, yyyy h:mm a')} · {script.word_count?.toLocaleString()} words
                  </p>
                </div>
              </div>
