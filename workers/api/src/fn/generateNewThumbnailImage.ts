// Ported from the original hosted generateNewThumbnailImage function
...
const handler: FnHandler = async (body, ctx) => {
  ...
  try {
    ...
    if (!concept_id) throw new HttpError(400, 'concept_id is required');
    ...
    // 1. Load concept
    let concept;
    try { concept = await ctx.db.ThumbnailConcepts.get(concept_id); }
    catch (e) { throw new HttpError(404, `Could not load concept: ${e.message}`); }
    if (!concept?.image_prompt) throw new HttpError(400, 'Concept has no image_prompt');
    ...
    if (!taskId) {
      throw new HttpError(500, `Task creation failed: ${createText.substring(0, 200)}`);
    }
    ...
    if (timeLeft() > 8000) {
      try {
        ...
        if (imgResp.ok) {
          ...
          const stored = await putMedia(ctx, imgBytes, {
            tier: 'durable',
            prefix: 'thumbnails',
            filename: `${concept_id}-${Date.now()}.${ext}`,
            contentType,
          });
