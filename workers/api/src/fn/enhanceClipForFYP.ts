...
// Ported from base44/functions/enhanceClipForFYP/entry/entry.ts
...
import { anthropicFetch, hasAiProvider } from '../lib/ai';
...
async function callClaude(ctx, systemPrompt: string, userPrompt: string) {
...
const handler: FnHandler = async (body, ctx) => {
  try {
    ...
    const { clip, words, video_duration, niche = '' } = body;

    if (!clip || !words?.length) {
      throw new HttpError(400, 'clip and words required');
    }
