...
// Ported from the original hosted sleepBrollPopulate function
...
const handler: FnHandler = async (body, ctx) => {
  try {
    ...
    if (scenes.length === 0) {
      return { success: true, populated: 0, message: 'No scenes found' };
    }
    ...
    for (let batchStart = 0; batchStart < scenes.length; batchStart += BATCH_SIZE) {
      ...
      const searchPromises = batch.map(async (scene, batchIdx) => {
        ...
        const [pexP, pixP, pexA, pixA] = await Promise.all([
          searchPexels(ctx, primary, orientation),
          searchPixabay(ctx, primary),
          searchPexels(ctx, alternative, orientation),
          searchPixabay(ctx, alternative),
        ]);
      ...
      for (const { scene, videos, query, sleepType } of batchResults) {
        ...
        if (!best || !best.downloadUrl) {
          ...
          const [fbPex, fbPix] = await Promise.all([
            searchPexels(ctx, fallbackQuery, orientation),
            searchPixabay(ctx, fallbackQuery),
          ]);
