// Ported from the original hosted cleanScenePrompt function
 ...
 */
export async function cleanPrompt(ctx, messyPrompt, visualStyle) {
 ...
 */
export async function cleanPromptsBatch(ctx, scenes, visualStyle, concurrency = 5) {
  ...
  for (let i = 0; i < scenes.length; i += concurrency) {
    ...
    const chunkResults = await Promise.all(
      chunk.map(async (s) => {
        try {
          const cleaned = await cleanPrompt(ctx, s.image_prompt, visualStyle);
...
const handler: FnHandler = async (body, ctx) => {
  try {
    const { prompt, prompts, visual_style } = body;

    // Single prompt mode
    if (prompt) {
      const cleaned = await cleanPrompt(ctx, prompt, visual_style);
      return { cleaned_prompt: cleaned };
    }

    // Batch mode: [{scene_number, image_prompt}, ...]
    if (prompts && Array.isArray(prompts)) {
      const results = await cleanPromptsBatch(ctx, prompts, visual_style);
      const output = prompts.map(p => ({
        scene_number: p.scene_number,
        cleaned_prompt: results.get(p.scene_number) || p.image_prompt
      }));
      return { cleaned_prompts: output };
    }
