...
// Ported from base44/functions/clipAndVoice/entry.ts
...
async function start(ctx, body, key) {
  ...
  const plan = await invokeLLM(ctx, {
  ...
  const [voiceTask, musicTask, ...sfxTasks] = await Promise.all([
    ...
    ...(plan.sfx || []).slice(0, 2).map(item => ai33('/v1s/task/music-generation', key, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        create_mode: 'simple',
        gpt_description_prompt: String(item.prompt).slice(0, 500),
...
const handler: FnHandler = async (body, ctx) => {
  try {
    const user = ctx.user;
    const key = await ctx.keys.get('AI33_API_KEY');
    if (!key) throw new HttpError(500, 'AI33_API_KEY not configured');

    return body.action === 'poll' ? await poll(body, key) : await start(ctx, body, key);
  } catch (error) {
