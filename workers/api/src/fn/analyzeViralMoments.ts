...
// Ported from base44/functions/analyzeViralMoments/entry.ts
...
import { anthropicFetch, geminiFetch, hasAiProvider, invokeLLM } from '../lib/ai';
...
// ── Gemini (primary) ────────────────────────────────────────────
async function callGemini(ctx, systemPrompt, userPrompt) {
  if (!(await hasAiProvider(ctx, 'gemini'))) throw new Error('GEMINI_API_KEY not configured');

  const response = await geminiFetch(ctx, `/v1beta/models/${GEMINI_MODEL}:generateContent`, {
    method: 'POST',
...
// ── Claude (fallback) ───────────────────────────────────────────
async function callClaude(ctx, systemPrompt, userPrompt) {
...
async function callBase44LLM(ctx, systemPrompt, userPrompt) {
  const result = await invokeLLM(ctx, {
    prompt: `${systemPrompt}\n\n${userPrompt}`,
...
async function callAI(ctx, systemPrompt, userPrompt) {
  const errors = [];

  // 1. Base44 built-in LLM first
  try {
    console.log(`🟣 Trying Base44 built-in LLM...`);
    const result = await callBase44LLM(ctx, systemPrompt, userPrompt);
  ...
  try {
    ...
    const result = await callGemini(ctx, systemPrompt, userPrompt);
  ...
  try {
    ...
    const result = await callClaude(ctx, systemPrompt, userPrompt);
...
const handler: FnHandler = async (body, ctx) => {
  try {
    ...
    if (!words?.length) {
      throw new HttpError(400, 'words array required (from ASR)');
    }
...
Sort clips by virality_score descending (best first).`;
    ...
    const { result, model_used } = await callAI(ctx, systemPrompt, userPrompt);
