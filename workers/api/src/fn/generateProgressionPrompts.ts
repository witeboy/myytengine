// Ported from base44/functions/generateProgressionPrompts/entry/entry.ts
...
import { openaiFetch } from '../lib/ai';
...
async function callLLM(ctx, prompt, temperature = 0.7) {
  const apiKey = await ctx.keys.get('OPENAI_API_KEY');
  if (!apiKey) throw new Error("OPENAI_API_KEY not configured");
  for (let attempt = 0; attempt < 3; attempt++) {
    const response = await openaiFetch(ctx, '/v1/chat/completions', {
      method: "POST",
...
const handler: FnHandler = async (body, ctx) => {
  try {
    ...
    if (!project_id || !title || !category) throw new HttpError(400, 'project_id, title, category required');
...
- environment_lock MUST match the location in "${subject_description || title}"`;
    ...
    if (!descriptions || descriptions.length < 7) {
      ...
      throw new HttpError(500, `Expected 7 descriptions, got ${descriptions?.length || 0}`);
    }
    ...
    try {
      await ctx.db.Projects.update(project_id, { status: 'breakdown_complete', current_step: 5 });
    } catch (updErr) {
