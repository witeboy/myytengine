...
// Ported from the original hosted newThumbnailConcept function
...
const handler: FnHandler = async (body, ctx) => {
  try {
    ...
    if (!video_title?.trim()) {
      throw new HttpError(400, 'video_title is required');
    }
    ...
    try {
      ...
      // Priority 2: Look up UploadMetadata by project_id
        ...
        const metaList = await ctx.db.UploadMetadata.filter({ project_id });
        if (metaList[0]) {
...
Do NOT add any other text.`;
        ...
        const record = await ctx.db.ThumbnailConcepts.create({
    ...
    if (!saved.length) {
      throw new HttpError(500, `All concepts failed to save. Error: ${failed[0]?.error}`);
    }
