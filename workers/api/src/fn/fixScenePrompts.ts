// Ported from the original hosted fixScenePrompts function
...
const styleCharacterRules = {
  ...
  gothic_candlelight: (b, f) => `fantasy ${b}, ${f}, warm candlelit tones, magical golden particles, gothic atmosphere`,
...
const handler: FnHandler = async (body, ctx) => {
  try {
    ...
    let scenesToFix;
    if (scene_id) {
      scenesToFix = await ctx.db.Scenes.filter({ id: scene_id });
    } else {
    ...
    for (const scene of scenesToFix) {
      ...
      if (result.changed || shouldReset || maskChanged) {
        ...
        if (Object.keys(update).length > 0) {
          await ctx.db.Scenes.update(scene.id, update);
          fixed++;
