...
// Ported from the original hosted parseAndScheduleTopics function
...
const handler: FnHandler = async (body, ctx) => {
  try {
    ...
    const allTopics = await ctx.db.ChannelTopics.filter({ channel_id });
    const unscheduled = allTopics
      .filter(t => !t.scheduled_date && t.status === 'queued')
      .sort((a, b) => (a.priority || 0) - (b.priority || 0));

    if (unscheduled.length === 0) {
      return { success: true, message: 'No topics to schedule', scheduled: 0 };
    }
    ...
    const geminiApiKey = await hasAiProvider(ctx, 'gemini');
    const youtubeApiKey = await ctx.keys.get('YOUTUBE_API_KEY');
...
Every topic (0-indexed) must appear exactly once in the plan.`;
      ...
      for (const item of aiPlan.plan) {
        if (typeof item.index === 'number' && item.index < unscheduled.length) {
          ...
          if (Object.keys(updateData).length > 0) {
            await ctx.db.ChannelTopics.update(topic.id, updateData);
          }
        }
      }
    }
    ...
    for (const update of updates) {
      ...
      await ctx.db.ChannelTopics.update(id, data);
    }
