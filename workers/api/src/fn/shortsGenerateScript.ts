// Ported from base44/functions/shortsGenerateScript/entry/entry.ts
...
import { anthropicFetch } from '../lib/ai';
...
async function callClaude(ctx, prompt, temperature = 0.75) {
...
const handler: FnHandler = async (body, ctx) => {
  try {
    ...
    const result = await callClaude(ctx, prompt, 0.75);
    ...
    await Promise.all(oldScripts.map(s => ctx.db.Scripts.delete(s.id).catch(() => {})));

    const newScript = await ctx.db.Scripts.create({
      project_id,
    ...
    return { success: true, title, word_count: wordCount, niche: shortsNiche };
