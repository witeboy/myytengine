// Ported from the original hosted generateProgressionVideo function
...
const handler: FnHandler = async (body, ctx) => {
  ...
  try {
    ...
    if (!KIE_API_KEY) throw new HttpError(500, 'KIE_API_KEY missing');

    // Fetch both scenes
    const startScenes = await ctx.db.Scenes.filter({ id: start_scene_id });
    const startScene = startScenes[0];
    if (!startScene?.image_url?.startsWith('http')) {
      throw new HttpError(400, 'Start scene has no valid image URL');
    }

    const endScenes = await ctx.db.Scenes.filter({ id: end_scene_id });
    const endScene = endScenes[0];
    if (!endScene?.image_url?.startsWith('http')) {
      throw new HttpError(400, 'End scene has no valid image URL');
    }
    ...
    await ctx.db.Scenes.update(start_scene_id, {
