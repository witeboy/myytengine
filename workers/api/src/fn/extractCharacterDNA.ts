// Ported from the original hosted extractCharacterDNA function
...
import { anthropicFetch, geminiFetch } from '../lib/ai';
...
// ═══ CLAUDE — BACKUP ═══
async function callClaude(ctx, prompt, temperature = 0.4) {
  const apiKey = await ctx.keys.get('ANTHROPIC_API_KEY');
...
const handler: FnHandler = async (body, ctx) => {
  try {
    ...
    if (
      ...
      return { success: true, skipped: true, reason: 'sleep_project' };
    }
...
- Maximum 5 characters. Merge background characters into archetypes if there are many.`;
    ...
    const { result, model } = await callAI(ctx, prompt, 0.4);
