// Ported from base44/functions/generateFullScript/entry/entry.ts
...
import { geminiFetch, hasAiProvider, openai } from '../lib/ai';
...
// ─── OpenAI helper ────────────────────────────────────────────────
async function callOpenAI(ctx, prompt, temperature = 0.4, retries = 3) {
  for (let attempt = 0; attempt < retries; attempt++) {
    try {
      const response = await openai(ctx, '/v1/chat/completions', {
        model: 'gpt-4o',
        temperature,
        max_tokens: 16384,
...
async function fetchGroundedResearch(ctx, topicTitle, niche) {
  if (!(await hasAiProvider(ctx, 'gemini'))) { console.warn('[research] No Gemini provider available'); return null; }
...
Provide 6-10 facts, 6-10 key numbers, 2-4 misconceptions. Be specific and accurate.`;
  ...
  try {
    const resp = await geminiFetch(ctx, '/v1beta/models/gemini-2.0-flash-exp:generateContent', {
        method: 'POST',
...
async function handleInitExplainer(ctx, project_id) {
  ...
  if (!hasUsableResearch(researchNotes)) {
    ...
    const fetched = await fetchGroundedResearch(ctx, topic?.title || project.name, project.niche);
    if (fetched && (fetched.facts?.length || fetched.key_numbers?.length)) {
      researchNotes = JSON.stringify(fetched);
      await ctx.db.Projects.update(project_id, { research_notes: researchNotes });
      console.log(`[init_explainer] ✅ Research: ${fetched.facts?.length || 0} facts, ${fetched.key_numbers?.length || 0} numbers`);
    }
  }

  // Clear old batches
  const existingBatches = await ctx.db.ScriptBatches.filter({ project_id });
  for (const batch of existingBatches) {
    await ctx.db.ScriptBatches.delete(batch.id);
  }
  ...
  const outlineResult = await callOpenAI(ctx, 
  ...
  for (let i = 0; i < 6; i++) {
    ...
    const batch = await ctx.db.ScriptBatches.create({
      project_id,
      batch_number:  i + 1,
...
async function handleMerge(ctx, project_id) {
  const projects = await ctx.db.Projects.filter({ id: project_id });
  const project  = projects[0];
  if (!project) throw new HttpError(404, 'Project not found');
  ...
  const allBatches = await ctx.db.ScriptBatches.filter({ project_id });
  const batches = allBatches
    .sort((a, b) => a.batch_number - b.batch_number)
    .filter(b => b.status === 'completed' && b.content);

  if (!batches.length) {
    throw new HttpError(400, 'No completed batches found to merge.');
  }
  ...
  const oldScripts    = await ctx.db.Scripts.filter({ project_id });
  const existingFinal = oldScripts.find(s => s.version === 'final_aggregated');

  let script;
  if (existingFinal) {
    await ctx.db.Scripts.update(existingFinal.id, {
    ...
    script = await ctx.db.Scripts.create({
      project_id,
...
const handler: FnHandler = async (body, ctx) => {
  try {
    ...
    if (mode === 'init_explainer') {
      return await handleInitExplainer(ctx, project_id);
    }

    // Default: merge
    return await handleMerge(ctx, project_id);
