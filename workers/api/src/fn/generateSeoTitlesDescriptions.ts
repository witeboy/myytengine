...
// Ported from the original hosted generateSeoTitlesDescriptions function
...
import { geminiFetch, hasAiProvider } from '../lib/ai';
...
async function callGemini(ctx, prompt, maxTokens = 4096) {
...
// ══════════════════════════════════════════════════════════════════
async function generateTagsFromScript(ctx, scriptText, niche, videoTitle, seoAnalysis) {
...

{
  ...
  const raw = await callGemini(ctx, prompt, 3000);
...
// ══════════════════════════════════════════════════════════════════
async function generateHashtagsFromScript(ctx, scriptText, niche, videoTitle, channelName) {
...

{
  ...
  const raw = await callGemini(ctx, prompt, 1500);
  return parseJson(raw);
}
...
const handler: FnHandler = async (body, ctx) => {
  try {
    ...
    if (!(await hasAiProvider(ctx, 'gemini'))) throw new HttpError(500, 'GEMINI_API_KEY not configured');
    ...
    if (project.selected_topic_id) {
      const topics = await ctx.db.Topics.filter({ id: project.selected_topic_id });
      if (topics[0]) {
    ...
    try {
      const thumbConcepts = await ctx.db.ThumbnailConcepts.filter({ project_id });
...
- Return ONLY the JSON object — no backticks, no explanation`;
    ...
    const titleResponseText = await callGemini(ctx, titlePrompt, 4096);
    ...
    if (!titleParsed?.titles?.length) {
      ...
      throw new HttpError(500, 'Failed to parse title response');
    }
    ...
    const tagResult = await generateTagsFromScript(ctx, fullScript, niche, titles[0]?.title || videoTitle, seoAnalysis);
    ...
    const hashResult = await generateHashtagsFromScript(ctx, fullScript, niche, titles[0]?.title || videoTitle, channelName);
    ...
    if (existingMeta.length > 0) {
      await ctx.db.UploadMetadata.update(existingMeta[0].id, metaData);
    } else {
      await ctx.db.UploadMetadata.create(metaData);
    }
    ...
    throw new HttpError(500, error.message || 'Title generation failed');
  }
};
