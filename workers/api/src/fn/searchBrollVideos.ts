...
// Ported from the original hosted searchBrollVideos function
...
async function searchPexels(ctx, searchTerms, duration, quality, orientation) {
...
async function searchPixabay(ctx, searchTerms, duration, quality) {
...
const handler: FnHandler = async (body, ctx) => {
  try {
    ...
    if (enabledSources.includes('pexels'))  searchPromises.push(searchPexels(ctx, searchTerms, duration, quality, pexelsOrientation));
    if (enabledSources.includes('pixabay')) searchPromises.push(searchPixabay(ctx, searchTerms, duration, quality));
