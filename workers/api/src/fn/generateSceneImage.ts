// Ported from the original hosted generateSceneImage function
...
// ─────────────────────────────────────────────
// KIE SEEDREAM — SUBMIT ONLY
// ─────────────────────────────────────────────

async function submitSeedream(apiKey, prompt, aspectRatio) {
  console.log(`🌱 KIE Seedream: submitting (${prompt.length} chars, ratio=${aspectRatio})...`);
  return kieCreateTask(apiKey, 'seedream-4.5', {
    prompt: prompt.substring(0, SEEDREAM_MAX_PROMPT_CHARS),
...
async function processScene(ctx, scene, project, kieApiKey, aspectRatio, referenceImageUrl, providerPref = 'auto') {
  ...
  if (isSleepProject) {
    ...
    finalPrompt = preparePromptForProvider(finalPrompt, 'seedream', true);
  ...
  if (wasFailedBefore && providerPref === 'auto') {
    console.log(`🔄 Scene ${sceneNum}: previously failed — trying ${providers.join(' → ')}`);
  } else {
  ...
  try {
    await ctx.db.Scenes.update(scene.id, { status: "image_failed" });
  } catch (_) {}
...
const handler: FnHandler = async (body, ctx) => {
  try {
    ...
    if (!KIE_API_KEY) {
      throw new HttpError(500, "No image API keys configured");
    }
    ...
    if (scene_id) {
      ...
      if (!scenes[0]) throw new HttpError(404, "Scene not found");
      ...
      const allScenes = await ctx.db.Scenes.filter({ id: scene_ids[0] });
      if (!allScenes[0]) throw new HttpError(404, "No scenes found");
      const pid = allScenes[0].project_id;
      const projectScenes = await ctx.db.Scenes.filter({ project_id: pid });
      scenesToProcess = projectScenes
        .filter(s => scene_ids.includes(s.id))
        .sort((a, b) => a.scene_number - b.scene_number);
      const projects = await ctx.db.Projects.filter({ id: pid });
      project = projects[0];

    } else if (project_id) {
      const projectScenes = await ctx.db.Scenes.filter({ project_id });
      scenesToProcess = projectScenes
      ...
      throw new HttpError(400, "Provide scene_id, scene_ids, or project_id");
    }
