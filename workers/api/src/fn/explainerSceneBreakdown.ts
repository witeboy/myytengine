// Ported from the original hosted explainerSceneBreakdown function
...
async function callGemini(ctx, prompt, systemText, temperature = 0.4) {
...
async function callClaudeFallback(ctx, prompt, systemText, temperature = 0.4) {
...
async function callAI(ctx, prompt, temperature = 0.4) {
  ...
  try { return await callGemini(ctx, prompt, systemText, temperature); }
  catch (geminiErr) {
    console.warn(`⚠️ Gemini failed: ${geminiErr.message.substring(0, 120)} — falling back to Claude`);
  }
  return await callClaudeFallback(ctx, prompt, systemText, temperature);
}
...
const handler: FnHandler = async (body, ctx) => {
  ...
  try {
    ...
    if (project.project_mode !== 'explainer') {
      throw new HttpError(400, `Project mode is "${project.project_mode || 'unset'}", not "explainer".`);
    }
    ...
    if (start_section === 0) {
      ...
      if (oldScenes.length > 0) {
        await Promise.all(oldScenes.map(s =>
          ctx.db.Scenes.delete(s.id).catch(() => {})
        ));
        console.log(`🗑️ Deleted ${oldScenes.length} old scenes`);
      }
    } else {
      const existing = await ctx.db.Scenes.filter({ project_id });
    ...
    for (let si = start_section; si < endSection; si++) {
      ...
      let sectionResult;
      try {
        sectionResult = await callAI(ctx, prompt, temp);
      } catch (err) {
    ...
    if (isDone) {
      const allScenes = await ctx.db.Scenes.filter({ project_id });
