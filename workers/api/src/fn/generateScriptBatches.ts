// Ported from the original hosted generateScriptBatches function
...
async function callLLM(ctx, prompt, temperature = 0.85) {
  try {
    ...
    if (!(await hasAiProvider(ctx, 'gemini'))) throw claudeErr;
...
`;
    ...
    if (pendingBatches.length === 0) {
      return { success: true, message: 'No pending batches to generate', completed: 0, done: true };
    }
    ...
    const batch = pendingBatches[0];
    {
      ...
      for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
        ...
        const { result, provider } = await callLLM(ctx, currentPrompt, baseTemp);
