// Ported from base44/functions/longViralGenerateScript/entry/entry.ts
...
// ── Claude API caller ────────────────────────────────────────────
async function callClaude(ctx, prompt, temperature = 0.85, retries = 2) {
...
async function callGemini(ctx, prompt, temperature = 0.85, retries = 2) {
  for (let attempt = 0; attempt <= retries; attempt++) {
...
// ── Unified LLM caller ──────────────────────────────────────────
async function callLLM(ctx, prompt, temperature = 0.85) {
  const hasAnthropic = await hasAiProvider(ctx, 'anthropic');
  const hasGemini = await hasAiProvider(ctx, 'gemini');
  // Try Claude first
  if (hasAnthropic) {
    try {
...
const handler: FnHandler = async (body, ctx) => {
  try {
    ...
    if (project.selected_hook_id) {
      const hooks = await ctx.db.Hooks.filter({ id: project.selected_hook_id });
...
- CTA Style: ${strat.cta_style || strat.cta || 'N/A'}`;
    ...
    let allBatches = await ctx.db.ScriptBatches.filter({ project_id });
    ...
    if (allBatches.length === 0) {
      ...
      if (outline.length > 0) {
        ...
        await ctx.db.ScriptBatches.bulkCreate(batchesToCreate);
        allBatches = await ctx.db.ScriptBatches.filter({ project_id });
  ...
  "batches": [
    {
      ...
      "synopsis": "Detailed 2-3 sentence synopsis of what to write"
        ...
        const { result: structureResult } = await callLLM(ctx, structurePrompt, 0.7);
        const generatedBatches = structureResult.batches || [];

        if (generatedBatches.length === 0) {
          throw new HttpError(500, 'Failed to generate script structure');
        }
    ...
    if (pendingBatches.length === 0) {
      ...
      const existingScripts = await ctx.db.Scripts.filter({ project_id });
      ...
      let script;
      if (existingFinal) {
        await ctx.db.Scripts.update(existingFinal.id, scriptData);
        script = { ...existingFinal, ...scriptData };
      } else {
        script = await ctx.db.Scripts.create(scriptData);
      }
    ...
    await ctx.db.ScriptBatches.update(batch.id, { status: 'generating' });
...
{"content": "The additional continuation text only...", "word_count": ${wordsNeeded}}`;
      ...
      const { result, provider } = await callLLM(ctx, currentPrompt, 0.85);
    ...
    await ctx.db.ScriptBatches.update(batch.id, {
      content,
      word_count: wordCount,
      status: 'completed',
    });
    ...
    if (allDone) {
      ...
      const freshBatches = await ctx.db.ScriptBatches.filter({ project_id });
