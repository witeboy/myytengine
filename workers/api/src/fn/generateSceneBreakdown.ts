// Ported from the original hosted generateSceneBreakdown function
...
async function callClaudeFallback(ctx, prompt, systemText, temperature = 0.7) {
  if (!(await hasAiProvider(ctx, 'anthropic'))) throw new Error("Missing Anthropic provider");
...
async function callAI(ctx, prompt, temperature = 0.7) {
  ...
  // Try Gemini first
  try {
    const result = await callGemini(ctx, prompt, systemText, temperature);
...
const handler: FnHandler = async (body, ctx) => {
  ...
  try {
    ...
    if (body.__claude_passthrough) {
      ...
      // Try Gemini first
      if (await hasAiProvider(ctx, 'gemini')) {
        try {
          const geminiBody = {
            contents: [{ role: 'user', parts: [{ text: prompt }] }],
          ...
          const gRes = await geminiFetch(ctx, '/v1beta/models/gemini-2.5-pro:generateContent', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(geminiBody) });
      ...
      // Fallback to Claude
      if (!(await hasAiProvider(ctx, 'anthropic'))) throw new HttpError(500, 'Neither GEMINI_API_KEY nor ANTHROPIC_API_KEY available');
      ...
      if (!text) throw new HttpError(500, 'No text from Claude fallback');
    ...
    if (!project_id) {
      throw new HttpError(400, 'Missing required field: project_id');
    }
    ...
    if (project.project_mode === 'explainer') {
      ...
      return explainerSceneBreakdown({
        project_id,
    ...
    let scriptBeats = [];
    let avgSceneDuration;

    if (isSleep) {
      avgSceneDuration = getSleepAvgSceneDuration(durationMinutes);
      ...
      avgSceneDuration = (durationMinutes * 60) / totalTargetScenes;
    ...
    if (totalAllocatedScenes === 0) {
      throw new HttpError(400, 'Phase allocation produced zero scenes. Check video_duration_minutes on the project.');
    }

    const scriptChunks = splitScriptByPhase(finalScript, phases, scriptBeats);

    if (scriptChunks.length === 0) {
      throw new HttpError(400, 'Script could not be split into scene chunks. Script may be too short.');
    }
      ...
      if (oldScenes.length > 0) {
        for (let i = 0; i < oldScenes.length; i += 10) {
          await Promise.all(oldScenes.slice(i, i + 10).map(s =>
            ctx.db.Scenes.delete(s.id).catch(_ => {})
          ));
        }
...
NICHE SENSIBILITY: ${nicheProfile.visual_world} | ${nicheProfile.emotional_palette}`;
      ...
      const analysis = await callAI(ctx, analysisPrompt, 0.6);
      storyAnalysis = analysis.story_analysis || analysis;

      if (!storyAnalysis || typeof storyAnalysis !== 'object') {
        throw new HttpError(500, 'Story analysis returned invalid data. Try again.');
      }
      ...
      if (!psList[0]?.story_analysis) {
        throw new HttpError(400, 'Story analysis not found. Run batch 0 first.');
      }
      ...
      freshProject = (await ctx.db.Projects.filter({ id: project_id }))[0];
