...
// Ported from base44/functions/extractBestMoments/entry.ts
...
async function callGemini(ctx, prompt, maxTokens = 8192) {
...
const handler: FnHandler = async (body, ctx) => {
  try {
    ...
    if (!transcript || transcript.length < 200) {
      throw new HttpError(400, 'Transcript too short for moment extraction');
    }
...
- If the video has fewer than ${max_clips} viral moments, return fewer — quality over quantity`;
    ...
    const raw = await callGemini(ctx, prompt, 8192);
    let parsed;
    try { parsed = JSON.parse(raw); }
    catch (_) {
      const match = raw.match(/\{[\s\S]*\}/);
      if (match) parsed = JSON.parse(match[0]);
      else throw new HttpError(500, 'Failed to parse AI response');
    }
    ...
    return { success: true, clips, source_duration: duration };
  } catch (error) {
