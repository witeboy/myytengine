// Ported from the original hosted shortsSceneBreakdown function
...
// ── Gemini 1.5 Pro ───────────────────────────────────────────────────────────
async function callGemini(ctx, prompt, systemText, temperature = 0.5) {
  if (!(await hasAiProvider(ctx, 'gemini'))) throw new Error("Missing Gemini provider");

  const response = await geminiFetch(ctx, '/v1beta/models/gemini-1.5-pro:generateContent', {
      method: "POST",
...
async function callClaudeFallback(ctx, prompt, systemText, temperature = 0.5) {
...
const handler: FnHandler = async (body, ctx) => {
  ...
  try {
    ...
    for (let bi = 0; bi < beatChunks.length; bi++) {
      ...
      let subResult;
      try {
        subResult = await callAI(ctx, prompt, 0.5);
      } catch (err) {
