// Ported from base44/functions/generateProgressionImage/entry/entry.ts
...
const handler: FnHandler = async (body, ctx) => {
  try {
    ...
    const { scene_id, reference_image_url } = body;
    ...
    if (!scene.image_prompt) throw new HttpError(400, 'No image prompt');
