...
// Ported from base44/functions/generateTopics.ts
...
const handler: FnHandler = async (body, ctx) => {
  try {
    ...
    if (!project_id || !niche) {
      throw new HttpError(400, 'Missing required fields: project_id, niche');
    }
...
IMPORTANT: Do NOT use special characters, line breaks, or unescaped quotes inside string values. Keep all string values simple and clean. Generate 5 topics ranked by viral potential.`;
    ...
    const result = await safeGeminiCall(ctx, prompt, 0.85);
    ...
    if (!result.data.topics || !Array.isArray(result.data.topics)) {
      throw new HttpError(500, 'Invalid response format from Gemini');
    }
    ...
    const savePromises = result.data.topics.map(async (topic, i) => {
      ...
      try {
        const record = await ctx.db.Topics.create({
