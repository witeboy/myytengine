...
// ══════════════════════════════════════════════════════════════════
// VOICEOVER GENERATOR — Single task, no chunking
// Submit: POST https://api.ai33.pro/v1m/task/text-to-speech
...
Deno.serve(async (req) => {
  try {
    ...
    console.log(`🎙 Voiceover: ${wordCount} words, ${cleanedText.length} chars, voice=${selectedVoiceId}, provider=${provider}`);

    // ── Submit single TTS task ──────────────────────────────────
