...
function getNicheDirectorProfile(niche) {
  ...
  // Robust fallback for any unlisted niche
...
Deno.serve(async (req) => {
  try {
    ...
    if (currentBatch === 0) {
      ...
      // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
      // STORY ANALYSIS PASS — Director reads the full script first
      // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
- "visual_world" should be SPECIFIC and sensory, not generic. Think: "Rain-streaked suburban windows, warm kitchen light spilling onto dark driveways, the weight of paper in weathered hands" — NOT "nice houses and offices."
- "recurring_visual_motifs" are visual threads that stitch scenes together. A clock, a doorway, light through a window, hands reaching — elements that echo and evolve.
- "color_arc" creates subliminal emotional continuity. The viewer won't notice it consciously but will FEEL it.
...
- "key_turning_points" should be EMOTIONAL shifts, not just topic changes. Where does the viewer's FEELING change?
...
`;
      ...
      // Save story analysis to project for use by all subsequent batches
      ...
      await base44.asServiceRole.entities.Projects.update(project_id, {
        ...
        scene_blueprint: JSON.stringify({
          ...
          scenes: [] // Will be populated by each batch
        }),
    ...
    // ── Load story analysis from project ───────────────────────────
    // Re-fetch project to get the latest scene_blueprint (especially for batch > 0)
    ...
    // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
    // SCENE BREAKDOWN PASS — Director blocks out the shots
    // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

    // Build the previous scenes context for continuity
    const previousScenes = blueprint.scenes.slice(-3); // Last 3 scenes for continuity
...
- Three sentences describing the same emotional moment = ONE scene with rich visual depth
- One sentence describing a dramatic shift = could be TWO scenes (the before and the after)
- Ask yourself: "Would a director call CUT here?" If not, it's the same scene.
...
The visual concept must be a SPECIFIC MOMENT frozen in time — not a general idea.

🎬 **RULE 3: SHOT VARIETY IS NON-NEGOTIABLE**
Never use the same shot type consecutively. Cycle through:
- ECU (Extreme Close-Up): Eyes, hands, objects, textures — intimate, emotional
- CU (Close-Up): Face, single object — personal, focused
- MCU (Medium Close-Up): Head and shoulders — conversational, connected
- MS (Medium Shot): Waist up — environmental context with human presence
- MWS (Medium Wide): Full body with environment — establishing relationship to space
- WS (Wide Shot): Full environment — context, scale, isolation or belonging
- EWS (Extreme Wide): Landscape/aerial — epic scope, insignificance or grandeur
- OTS (Over-the-Shoulder): Relationship between subjects — perspective, power dynamic
- INSERT/DETAIL: Macro object shot — symbolic weight, narrative emphasis
- LOW ANGLE: Looking up — power, hope, aspiration, threat
- HIGH ANGLE/OVERHEAD: Looking down — vulnerability, overview, divine perspective
- DUTCH ANGLE: Tilted frame — unease, disorientation, wrongness
- POV: Through someone's eyes — immersion, empathy

🎬 **RULE 4: EMOTIONAL ESCALATION WITHIN THE PHASE**
Even within a single phase, scenes should ESCALATE emotionally. Scene 1 of a phase should be less intense than the last scene. Build the wave.
...
- Same color accent appearing in different contexts
- A motif evolving (e.g. a closed door → a cracked door → an open door)
- Lighting shifting gradually (not jumping from dark to bright)
- Camera distance alternating rhythmically (wide → tight → wide → tight)

🎬 **RULE 6: ABSTRACT → CONCRETE**
When the narration is abstract ("the economy was failing"), the visual must be CONCRETE and PHYSICAL:
- "inflation" → close-up of a grocery receipt curling longer and longer, spilling off a kitchen counter
- "loneliness" → a single place setting at a table made for six
- "hope" → first crack of sunrise light hitting a dark room through curtains
NEVER visualize abstractions abstractly. Make them PHYSICAL, TANGIBLE, REAL.
  ...
  "scenes": [
    {
      ...
      "narration_text": "EXACT words from the script segment that play during this scene. Must use every word exactly once across all scenes.",
      "visual_concept": "A rich, specific, CINEMATIC description of what we SEE. Not a prompt — a director's vision note. 2-4 sentences describing the exact frozen moment, its composition, textures, and emotional weight.",
      "shot_type": "Specific shot type from the list above (e.g. 'ECU — Extreme Close-Up')",
      "camera_angle": "Specific angle (e.g. 'Low angle, 15 degrees, slightly left of center')",
      "camera_movement": "Specific movement (e.g. 'Slow push-in over 8 seconds, from MS to MCU')",
      "lighting": "Specific lighting setup (e.g. 'Single warm practical light from desk lamp, camera left. Deep shadows camera right. Subtle rim light from window behind.')",
      "color_palette": "3-4 specific colors (e.g. 'Warm amber #D4A574, deep shadow brown #2C1810, cream highlight #F5F0E8')",
      "mood": "Emotional tone in 2-3 words (e.g. 'quiet desperation', 'fragile hope', 'building dread')",
      "depth_of_field": "Specific DOF (e.g. 'Shallow f/1.4 — subject sharp, background melted to bokeh')",
      "niche_visual_element": "One specific visual element drawn from the niche's metaphor language that reinforces the theme",
      "continuity_bridge": "What visual thread connects this scene to the NEXT scene (e.g. 'The warm light from the lamp carries into the next scene as sunrise light')",
...
- The visual_concept is the HEART of this response — make each one a vivid, specific, filmable moment
...
`;
    ...
    if (result.scenes && Array.isArray(result.scenes)) {
      // Validate shot variety — no consecutive duplicates
      for (let i = 1; i < result.scenes.length; i++) {
        if (result.scenes[i].shot_type === result.scenes[i - 1].shot_type) {
          ...
          // Don't block — log for monitoring. The prompt handles this.
        }
      }
      ...
      for (const scene of result.scenes) {
        ...
        // Save scene record with status "breakdown_ready"
        // (prompt generator will pick these up and add image_prompt/animation_prompt)
        await base44.asServiceRole.entities.Scenes.create({
          project_id,
          scene_number: sceneNum,
          narration_text: cleanedNarration,
          image_prompt: "", // Will be filled by prompt generator
          animation_prompt: "", // Will be filled by prompt generator
          duration_seconds: scene.duration_seconds || 8,
          status: "breakdown_ready"
        });

        // Store rich directorial data in blueprint
