...
// ── Build the writing prompt for a batch ─────────────────────────
function buildWritingPrompt({ batch, project, topic, selectedHook, sortedBatches, previousContent, outlineContext, isFirstBatch, isLastBatch, strategyBlock }) {
...
**MANDATORY WORD COUNT**: You MUST write AT LEAST ${batch.target_words} words. This is NON-NEGOTIABLE. If your output is under ${Math.round(batch.target_words * 0.9)} words, it is a FAILURE. Count your words carefully. Add more detail, more anecdotes, more specific examples, more emotional beats, deeper analysis until you hit the target. The video NEEDS this many words to fill its timeslot (150 words = 1 minute of narration).
...
Return ONLY valid JSON (no markdown):
{
  "batches": [
    ...
    // ── Generate with word count enforcement ─────────────────
