...
═══════════════════════════════════════
PART 3: TAGS — MAX 7 TOTAL (YouTube spam-safe cap)
═══════════════════════════════════════

Generate ONLY the 7 BEST tags split across 3 tiers:
- 2 SHORT (1-2 words): Broadest category. High volume.
- 2 MEDIUM (3-4 words): Core intent. Moderate competition.
- 3 LONG-TAIL (5-8 words): Specific queries. Low competition, high intent.

All must be: actually searched by real people, directly relevant to THIS video.
Pick only the highest-intent, most-targeted tags. Do NOT generate more than 7 —
keyword stuffing hurts the algorithm.
...

{
  ...
  "tags_short": ["tag1","tag2"],
  "tags_medium": ["medium tag 1","medium tag 2"],
  "tags_long": ["long tail phrase 1","long tail phrase 2","long tail phrase 3"],
...
Generate the complete premium SEO package now. Respond ONLY with the JSON object.`;
    ...
    // ══════════════════════════════════════════════════════════════
    // BUILD TAG ARRAYS — capped at 7 total across all tiers
    // YouTube spam-avoidance: too many tags can flag videos as keyword-stuffed.
    // We keep the best 7 by picking a balanced mix: 2 short + 2 medium + 3 long-tail.
    // ══════════════════════════════════════════════════════════════
    const TAG_CAP = 7;
    const rawShort = (d.tags_short || []).slice(0, 2);
    const rawMedium = (d.tags_medium || []).slice(0, 2);
    const rawLong = (d.tags_long || []).slice(0, 3);
    let allTags = [...rawShort, ...rawMedium, ...rawLong];
    // If any tier was empty, backfill from the others to always hit up to TAG_CAP
    if (allTags.length < TAG_CAP) {
      const pool = [...(d.tags_long || []), ...(d.tags_medium || []), ...(d.tags_short || [])];
      for (const t of pool) {
        if (allTags.length >= TAG_CAP) break;
        if (!allTags.includes(t)) allTags.push(t);
      }
    }
    allTags = allTags.slice(0, TAG_CAP);
    // Keep per-tier arrays aligned to the final selection for downstream UI
    const tagsShort = allTags.filter(t => (d.tags_short || []).includes(t));
    const tagsMedium = allTags.filter(t => (d.tags_medium || []).includes(t));
    const tagsLong = allTags.filter(t => (d.tags_long || []).includes(t));
