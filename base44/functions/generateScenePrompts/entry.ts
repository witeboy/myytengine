...
async function callOpenAI(prompt, systemText, temperature = 0.7, maxTokens = 8192) {
  ...
  if (!rawText) {
    ...
    throw new Error(`OpenAI returned no text. finish_reason: ${finishReason}`);
  }

  const parsed = extractJSON(rawText);
  if (parsed) return parsed;

  console.error(`❌ OpenAI JSON parse failed. Sample: ${rawText.substring(0, 500)}`);
  throw new Error(`Failed to parse OpenAI JSON. Length: ${rawText.length} chars.`);
}
...
async function callClaudeFallback(prompt, systemText, temperature = 0.7, maxTokens = 8192) {
...
// ══════════════════════════════════════════════════════════════════
// Unified AI caller: OpenAI first → Claude fallback
// ══════════════════════════════════════════════════════════════════

async function callAI(prompt, temperature = 0.7, maxTokens = 8192) {
  const systemText = "You are a world-class film director and cinematic data extractor. You must return ONLY raw, valid JSON. Do not include markdown formatting like ```json and do not include any conversational text.";

  // Try OpenAI first
  try {
    const result = await callOpenAI(prompt, systemText, temperature, maxTokens);
    console.log(`✅ OpenAI succeeded`);
    return result;
  } catch (openaiErr) {
    console.warn(`⚠️ OpenAI failed: ${openaiErr.message} — falling back to Claude`);
  }

  // Fallback to Claude Sonnet 3.5
  const result = await callClaudeFallback(prompt, systemText, temperature, maxTokens);
...
Deno.serve(async (req) => {
  try {
    ...
    console.log(`━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━`);
    console.log(`🎨 PROMPT GENERATION (OpenAI primary, Claude fallback)`);
    ...
    const genrePreset = resolveGenrePreset(projectMode, storyArch);
...
`;
    ...
    const framingPrefix = genrePreset.prefix;
    ...
    for (const c of characters) {
      ...
      identityDesc = identityDesc
        ...
        .replace(/\(\s*Beige\s*\d*\s*\)/gi, match => match)
      ...
      if (name && identityDesc) {
        ...
        const charIsMale = hasExplicitMale && !hasExplicitFemale ? true
          ...
          : hasExplicitMale && hasExplicitFemale ? true
          : true;
    ...
    console.log(`🖼️ Style: ${visualStyle} | 📐 ${orientation} | 🤖 AI: OpenAI→Claude`);
...
- **Low Angle:** Camera below subject's eyeline, shooting upward. Makes subject powerful, heroic, intimidating.
- **High Angle:** Camera above, looking down. Makes subject vulnerable, small, overwhelmed.
- **Dutch Angle (Canted):** Camera tilted to one side. Creates unease, psychological distress, disorientation.
- **Over-the-Shoulder (OTS):** Shot from behind one person looking at another or at something.
- **Eye-Level:** Neutral, at subject's height. Objective, realistic, documentary feel.
- **Bird's-Eye (Overhead):** Directly above the scene. Emphasizes environment scale, isolation.
- **Point-of-View (POV):** Shows exactly what the character sees.

**Core cinematography rules (ENFORCE in every prompt):**
- **180-Degree Rule:** In dialogue/confrontation scenes, maintain spatial consistency.
- **30-Degree Rule:** Never describe two consecutive scenes from nearly the same angle — shift at least 30°.
- **90-Degree Rule:** NEVER frame a character straight-on like a mugshot.

**SCENE CONSTRUCTION RULES:**
- **PLOT-FIRST:** Every scene must show what is ACTUALLY HAPPENING in the story at this moment.
- **STORY GROUNDED:** Show the character in their real situation performing real actions.
...
- Characters must be DOING something relevant to the PLOT
- Backgrounds must be SHARP and DETAILED
- **POPULATED WORLD:** Include other people in most scenes
- **NO ABSTRACT METAPHORS:** Every scene must be a plausible moment from the character's life.
...
1. **image_prompt** — Write each prompt as a CINEMATOGRAPHER would describe a shot to their crew.

   **HOW TO WRITE:**
   Write the way Roger Deakins talks about a shot — camera placement first, then what the lens DISCOVERS.
   WRONG: "Medium shot from waist up of a man in a dimly lit office."
   RIGHT: "Low angle through the dusty glass of a desk lamp — a gaunt figure hunches over scattered papers in a back-office thick with cigarette haze, the amber light carving deep shadows under ice-blue eyes."

   **WORD LIMIT:** 60-120 words per prompt.
   **FORBIDDEN:** "from waist up", "from chest up", feature catalogs, text on surfaces, screen content.

   Additional rules:
   • ${orientationConfig.composition}
   - Abstract concepts → show the CHARACTER experiencing the real-world version.

2. **animation_prompt** — RICH motion direction for the EXACT duration of each scene.
${useSleepStyle ? `   **🌙 SLEEP MODE ANIMATION — STRICT RULES:**
   - ONLY ALLOWED MOTION: ultra-slow camera pan/zoom, subtle nature motion, gentle depth breathing.
   - ⛔ FORBIDDEN: ANY light animation, sudden movement, subject motion, rack focus, dramatic language.
   - Keep to 1-2 calm sentences.` : `   - NOT a simple camera instruction — a FULL MOTION POEM.
   - **Include ALL layers:** camera, atmosphere, subject, light, depth, emotional quality.
   - **MINIMUM 3-4 rich sentences**`}
...
**RESPONSE:**
{
  "prompts": [
    {
      "scene_number": 1,
      "image_prompt": "[CAMERA ANGLE + what it discovers]. [Environment]. [Character identity woven into action]. [Mood + style suffix]",
      ...
      const result = await callAI(prompt, 0.7, 8192);
...
Write like a cinematographer. Lead with WHERE THE CAMERA IS, then what the lens discovers.
NEVER use "medium shot from waist up" or "from chest up".
End with mood and style: ${styleConfig.positive.substring(0, 100)}

Minimum 80 words. Respond with JSON: {"image_prompt": "..."}`;

              const soloResult = await callAI(soloPrompt, 0.8, 4096);
          ...
          if (namedProps.length > 0) {
            for (const prop of namedProps) {
              const genericProp = prop.replace(/^an?\s+/i, '');
              ...
              if (pattern && pattern.test(rawPrompt)) {
                ...
                console.log(`📱 Scene ${s.scene_number}: prop "${genericProp}" injected`);
              }
            }
          }

          // ═══ STRIP BARE CHARACTER NAMES ═══
          for (const c of characters) {
          ...
          if (!/\b(is|was|sits|stands|walks|runs|holds|stares|looks|leans|clutch|grip|reach|kneel|crouch|watch|gaze|turn|step|press|scroll|tap|delet|swip|carry|push|pull|lift|throw|pour|eat|drink|read|writ|typ|driv|sitting|standing|walking|holding|staring|leaning|scrolling|tapping|deleting|carrying|pushing)\w*\b/i.test(rawPrompt)) {
            ...
            console.log(`🎬 Scene ${s.scene_number}: injected action "${action}"`);
          }
          ...
          if (animationPrompt.length < 80) {
            ...
            if (useSleepStyle) {
              ...
              const povNote = /\bpov\b|point[\s-]of[\s-]view/i.test(s.director?.shot_type || '')
                ? ' POV SHOT: camera IS the character — slight head-bob on movement, natural eye-drift to points of interest, hands entering frame from below.'
                : '';
          ...
          if (useSleepStyle) {
            ...
            const fbEnvLayer = vc.includes('rain') ? 'Rain streaks every surface.'
              : vc.includes('wind') ? 'Wind is constant and directional.'
              : vc.includes('crowd') ? 'Background crowd moves at three different speeds.'
              : 'Ambient world physics are alive.';
            const fbMoodLayer = mood.includes('tense') ? 'Light source flickers.'
              : mood.includes('warm') ? 'Golden warmth expands from one edge.'
              : mood.includes('sad') ? 'Light recedes.'
              : 'Light shifts once, deliberately.';
            animationPrompt = fbDurationTier === 'snap'
              ? `${fbArcGuidance} ONE motion only.`
              : fbDurationTier === 'build'
              ? `${fbArcGuidance} ${fbEnvLayer}`
              : `${fbArcGuidance} ${fbEnvLayer} ${fbMoodLayer} Shallow DOF breathes toward the emotional focal point.`;
          }
        }
