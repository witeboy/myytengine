...
Deno.serve(async (req) => {
  try {
    ...
    // Attempt 1: Z-Image (primary — fast, high quality, renders text cleanly)
    let imageUrl = null;
    let model = 'none';

    try {
      console.log('[Z-Image] Generating tweaked thumbnail...');
      const zPrompt = `${processedPrompt.substring(0, 1800)}. Ultra high resolution 1920x1080 Full HD, crisp sharp details, professional quality.`;
      ...
      console.warn(`Z-Image failed: ${e.message}`);
    }

    // Attempt 2: Ideogram V3 QUALITY (fallback — best text rendering)
    if (!imageUrl) {
      try {
        console.log('[Ideogram V3] Fallback...');
