...
const SECTION_CADENCE = {
  ...
  // LIST mode — one entry per list item
  item:         { cuts_per_min: 11, min_scenes: 8,  max_scenes: 999, scene_dur: [2.0, 3.0] },
};
...
function computeSceneTarget(sectionType, durationMinutes, sectionPctOverride) {
  const cad = SECTION_CADENCE[sectionType] || SECTION_CADENCE.core_concept;
  const pct = sectionPctOverride ?? EXPLAINER_SECTION_TIME_PCT[sectionType] ?? (1 / 6);
...
function buildSectionBreakdownPrompt({
  ...
  const pacingDirective = sectionType === 'hook'
    ...
    : sectionType === 'item'
    ? `ITEM CADENCE: 11 cuts/min — this is one entry on a list. Open with a BIG TITLE CARD showing the item's name/number ("Number 3: Laundromats"). Then alternate Einstein explaining, chalkboard with dollar figures, and visual metaphors for the business. Scenes ${durLo}-${durHi}s. Make ~${targetSceneCount} scenes.`
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // For LIST videos: compute per-section pct from total batch counts
    // (hook + N items + takeaway). Distribution: hook 8%, takeaway 8%, items share 84%.
    const itemSectionCount = outlineSections.filter(s => s.section_type === 'item').length;
    const isListVideo = itemSectionCount > 0;
    const itemPct = isListVideo ? (0.84 / itemSectionCount) : null;
    ...
    for (let si = start_section; si < endSection; si++) {
      ...
      const pctOverride = isListVideo
        ? (sectionType === 'hook' || sectionType === 'takeaway' ? 0.08 : itemPct)
        : undefined;
      const { target, cadence, sectionMinutes } = computeSceneTarget(sectionType, durationMinutes, pctOverride);
