...
async function processScene(base44, scene, project, kieApiKey, ai33ApiKey, aspectRatio, referenceImageUrl, providerPref = 'auto') {
  ...
  if (scene.status === 'image_generated' && scene.image_url && !scene.image_url.startsWith('ai33_task:') && !scene.image_url.startsWith('grok_img_task:') && !scene.image_url.startsWith('nano_task:') && !scene.image_url.startsWith('z_image_task:')) {
  ...
  if (providerPref === 'auto') {
    // Default cascade: Z-Image (primary, fast + high quality) → AI33 → Grok → Nano
    // Z-Image doesn't support image-to-image references, so when character reference is needed,
    // it's pushed behind Grok (which does support references).
    providers = isSleepProject
      ? [kieApiKey ? 'z_image' : null, ai33ApiKey ? 'ai33_seedream' : null, 'nano_banana', 'grok'].filter(Boolean)
      : [kieApiKey ? 'z_image' : null, ai33ApiKey ? 'ai33_seedream' : null, 'grok', 'nano_banana'].filter(Boolean);
  } else {
    // User explicitly picked a provider — use it FIRST, then fallback to others
    const allProviders = ['z_image', 'ai33_seedream', 'grok', 'nano_banana'];
    providers = [providerPref, ...allProviders.filter(p => p !== providerPref)];
    // Filter out unavailable providers
    providers = providers.filter(p => {
      if (p === 'ai33_seedream' && !ai33ApiKey) return false;
      if ((p === 'grok' || p === 'nano_banana' || p === 'z_image') && !kieApiKey) return false;
      return true;
    });
  }
  ...
  for (const provider of providers) {
    try {
      ...
      if (provider === 'z_image') {
        // Z-Image: pure text-to-image, no reference support. Skip if we need a character reference.
        if (useReference) {
          throw new Error('z_image does not support reference images — skipping to next provider');
        }
        taskId = await kieCreateTask(kieApiKey, "z-image", {
          prompt: providerPrompt,
          aspect_ratio: aspectRatio
        });
        taskPrefix = 'z_image_task';
      } else if (provider === 'ai33_seedream') {
...
Deno.serve(async (req) => {
  try {
    ...
    // Validate
    if (!['auto', 'z_image', 'ai33_seedream', 'grok', 'nano_banana'].includes(providerPref)) providerPref = 'auto';
