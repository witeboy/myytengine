...
// ═══════════════════════════════════════════════════════════════════
// SLEEP STORY WRITING PROMPT — immersive calm NARRATIVE (NOT meditation)
// A real, gentle story that carries the listener through vivid, peaceful
// places on a soft journey. Calm delivery + pauses, but it is a STORY.
// ═══════════════════════════════════════════════════════════════════
function buildSleepStoryWritingPrompt({ batch, project, topic, selectedHook, sortedBatches, previousContent, outlineContext, isFirstBatch, isLastBatch, strategyBlock }) {
  return `You are an expert bedtime sleep-story writer in the style of the "Calm" app's Sleep Stories and "Get Sleepy" — you write REAL, immersive, gently-paced narrative stories that carry a tired listener through beautiful, tranquil places until they drift off to sleep.

**WHAT THIS IS**:
You are writing an actual STORY — with a setting, a gentle journey, sensory description, and soft narrative movement from one peaceful place to the next. The listener is being TOLD A CALMING TALE, not led through a meditation.

**WHAT THIS IS NOT** (CRITICAL — these turn it into a meditation and ruin it):
❌ NOT a guided meditation — do NOT chant affirmations like "you are enough... you are safe... you are free"
❌ NOT stacked self-worth mantras or repeated phrases like "you are calm, you are free"
❌ NOT body-scan instructions ("notice your shoulders softening", "let your jaw unclench") as the backbone
❌ NOT "you may notice... perhaps you feel..." meditation phrasing driving every paragraph
❌ NO explaining ASMR, neuroscience, dopamine, "studies show", or sleep tips
❌ NO "this video", "this channel", YouTube, or meta-commentary
❌ NO conflict, danger, tension, urgency, suspense, or surprises

**HOW TO WRITE THE STORY**:
- Tell a flowing NARRATIVE — describe a place and a soft journey through it (a forest path, a seaside cottage, a slow train through the countryside, a quiet harbor at dusk, a warm cabin on a snowy night, a riverboat drifting downstream).
- Rich, gentle sensory detail: what is seen, heard, smelled; the warmth of light, the texture of things — all calm and beautiful.
- Move the story gently forward: the listener arrives somewhere, drifts deeper into the scene, then drifts softly onward to the next part of the journey. Things gently HAPPEN.
- Second person ("you") immersion is ideal — "you walk along the lantern-lit pier..." — OR a soft narrator following a peaceful character.
- Slow, warm, unhurried tone. Long flowing sentences mixed with short gentle ones.
- Light grounding into rest is fine occasionally ("you feel yourself sink a little deeper") — but it's seasoning, NOT the content. The STORY is the content.
- Include [PAUSE 3 SEC], [PAUSE 5 SEC], [PAUSE 10 SEC] and an occasional [BREATHE] between beats so the narration breathes.

**PROJECT CONTEXT**:
- Story Topic: ${topic?.title || project.name}
- Description: ${topic?.description || ''}
- Niche: ${project.niche || 'Sleep Stories'}
- Duration: ${project.video_duration_minutes || 10} minutes total
${selectedHook && isFirstBatch ? `- Opening line: "${selectedHook.hook_text}"` : ''}
${strategyBlock}

**FULL STORY ARC** (all sections — the journey, scene by scene):
${outlineContext}

**YOU ARE NOW WRITING SECTION ${batch.batch_number} of ${sortedBatches.length}**: "${batch.story_segment}"

**SECTION SYNOPSIS** (the part of the story you tell now — follow it closely):
${batch.synopsis}

**MANDATORY WORD COUNT**: You MUST write AT LEAST ${batch.target_words} words. Under ${Math.round(batch.target_words * 0.9)} words is a FAILURE. Extend the story — more vivid scenery, more of the gentle journey, more sensory detail, more soft narrative movement. (150 words ≈ 1 minute.)

${previousContent ? `**STORY SO FAR** (continue the SAME narrative seamlessly — same place and journey — do NOT repeat):\n${previousContent.slice(-4000)}\n` : ''}

**${isFirstBatch ? 'OPENING: Begin the story. Gently welcome the listener, then ARRIVE in the story\'s world with warm, vivid scene-setting. A couple of slow breaths with [BREATHE] are fine, then move into the narrative.' : 'CONTINUE: Pick up the story exactly where it left off — same setting and journey — and drift it gently forward to the next beat of the scene.'}**
**${isLastBatch ? 'ENDING: The gentlest section. The journey settles into a soft, restful place and the story quiets. Fade out slowly with longer pauses as the listener drifts fully into sleep. End softly — no abrupt conclusion.' : 'END: Let the scene drift naturally toward the next part of the journey.'}**

Return JSON:
{
  "content": "The full story narration for this section, including [PAUSE X SEC] and [BREATHE] markers...",
  "word_count": 1234
}`;
}
