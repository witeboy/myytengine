...
// ── Transcript Tier 1: youtubetranscript.dev V2 ──────────────────
// Docs: https://www.youtubetranscript.dev (V2 API)
//   POST /api/v2/transcribe         → sync for captions, async job for ASR
//   GET  /api/v2/jobs/{job_id}      → poll ASR job until completed
async function getTranscriptAPI(videoId) {
  ...
  const BASE = 'https://www.youtubetranscript.dev/api/v2';
  const headers = {
    'Authorization': `Bearer ${apiKey}`,
    'Content-Type': 'application/json',
  };

  // Extract plain text from a V2 transcript data object
  const extractText = (data) => {
    const t = data?.transcript;
    if (!t) return null;
    if (typeof t.text === 'string' && t.text.trim().length > 50) return t.text.trim();
    if (Array.isArray(t.segments)) {
      const joined = t.segments.map(s => s.text || '').join(' ').replace(/\s+/g, ' ').trim();
  ...
  try {
    // Step 1: POST /transcribe with allow_asr so we get captions OR a queued ASR job
    console.log(`[Transcript T1] POST /transcribe for ${videoId}`);
    ...
    console.log(`[Transcript T1] Status: ${submitRes.status}`);
    const submitData = await submitRes.json().catch(() => ({}));

    if (!submitRes.ok) {
      console.log(`[Transcript T1] Error body: ${JSON.stringify(submitData).slice(0, 300)}`);
      return null;
    }

    // Case A: already completed synchronously (captions hit)
    if (submitData.status === 'completed' && submitData.data) {
      const text = extractText(submitData.data);
      if (text) {
        console.log(`[Transcript T1] ✅ Sync captions: ${text.length} chars (source=${submitData.data.transcript?.source})`);
        return text;
      }
    }

    // Case B: async ASR job — poll /jobs/{job_id}
    const jobId = submitData.job_id || submitData.data?.job_id || submitData.request_id;
    if (submitData.status === 'processing' && jobId) {
      console.log(`[Transcript T1] ⏳ ASR job queued: ${jobId}`);
      const DEADLINE_MS = 90_000; // 90s max
      const INTERVAL_MS = 5_000;
      const start = Date.now();

      while (Date.now() - start < DEADLINE_MS) {
        await new Promise(r => setTimeout(r, INTERVAL_MS));
        const pollRes = await fetch(`${BASE}/jobs/${jobId}`, { headers });
        if (!pollRes.ok) {
          console.log(`[Transcript T1] Poll ${jobId} status ${pollRes.status}`);
          continue;
        }
        const pollData = await pollRes.json().catch(() => ({}));
        console.log(`[Transcript T1] Poll ${jobId}: ${pollData.status}`);

        if (pollData.status === 'completed' && pollData.data) {
          const text = extractText(pollData.data);
          if (text) {
            console.log(`[Transcript T1] ✅ ASR done: ${text.length} chars`);
            return text;
          }
        }
        if (pollData.status === 'failed') {
          console.log(`[Transcript T1] ❌ ASR failed`);
          return null;
        }
      }
      console.log(`[Transcript T1] ⏱️ ASR poll timeout after ${DEADLINE_MS}ms`);
    }

    console.log(`[Transcript T1] No usable transcript. Response: ${JSON.stringify(submitData).slice(0, 300)}`);
    return null;
  } catch (e) {
