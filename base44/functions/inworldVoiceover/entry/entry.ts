...
function getR2PublicUrl() {
  return 'https://media.myvoicify.app';
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!project_id) return Response.json({ error: 'Missing project_id' }, { status: 400, headers: corsHeaders });

    const INWORLD_KEY = Deno.env.get('INWORLD_API_KEY');
    if (!INWORLD_KEY) return Response.json({ error: 'INWORLD_API_KEY not configured' }, { status: 500, headers: corsHeaders });
    ...
    if (action === 'start') {
      ...
      if (!script?.full_script) return Response.json({ error: 'No final_aggregated script found.' }, { status: 400, headers: corsHeaders });
    ...
    if (action === 'process') {
      ...
      if (pendingIndices.length === 0) {
        ...
        try {
          ...
          return Response.json({ status: 'ready', voiceover_url: fallbackUrl, concatenated: false }, { headers: corsHeaders });
        }
      }
    ...
    // ════════════════════════════════════════════════════════════
    if (action === 'status') {
      if (!settings) return Response.json({ status: 'idle' }, { headers: corsHeaders });

      if (settings.voiceover_status === 'completed' && settings.voiceover_url) {
        return Response.json({ status: 'ready', voiceover_url: settings.voiceover_url }, { headers: corsHeaders });
      }
    ...
    return Response.json({ error: 'Invalid action. Use start, process, or status.' }, { status: 400, headers: corsHeaders });
