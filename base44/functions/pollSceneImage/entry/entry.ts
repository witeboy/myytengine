...
// ══════════════════════════════════════════════════════════════════
// POLL SCENE IMAGE — checks pending tasks and resolves them (v2)
// ══════════════════════════════════════════════════════════════════
  ...
  try {
    ...
    for (const scene of scenesToPoll) {
      ...
      try {
        ...
        // ════════════════════════════════════════════════════════
        // Z-IMAGE POLL (via KIE) — uses same KIE endpoint as Grok/Nano
        // ════════════════════════════════════════════════════════
        if (imageUrl.startsWith('z_image_task:')) {
          const taskId = imageUrl.replace('z_image_task:', '');
          ...
          if (poll.data?.state === 'success') {
            ...
            if (finalUrl) {
              ...
              if (projectForRef && !projectForRef.reference_image_url && detectCharacterInScene(scene)) {
                try {
                  await base44.asServiceRole.entities.Projects.update(projectForRef.id, { reference_image_url: finalUrl });
                  projectForRef.reference_image_url = finalUrl;
                } catch (_) {}
              }

              console.log(`✅ Scene ${sceneNum}: Z-Image done → ${finalUrl.substring(0, 60)}`);
              results.push({ scene_number: sceneNum, status: 'done', image_url: finalUrl });
              continue;
            } else {
              await base44.asServiceRole.entities.Scenes.update(scene.id, { status: 'image_failed', image_url: '' });
              results.push({ scene_number: sceneNum, status: 'failed', error: 'Z-Image returned no URL' });
              continue;
            }
          }

          // ── FAIL — fallback to Grok ──
          if (poll.data?.state === 'fail') {
            const errMsg = poll.data?.failMsg || 'Z-Image failed';
            console.warn(`❌ Scene ${sceneNum}: Z-Image failed — ${errMsg} → Grok fallback`);
            const aspectRatio = projectForRef?.orientation === 'portrait' ? '9:16' : '16:9';
            const grokTaskId = await submitGrokFallback(KIE_API_KEY, scene, aspectRatio, projectForRef?.reference_image_url);
            if (grokTaskId) {
              await base44.asServiceRole.entities.Scenes.update(scene.id, { image_url: `grok_img_task:${grokTaskId}`, status: 'image_pending' });
          ...
          // ── STILL PROCESSING ──
          {
            ...
            console.log(`⏳ Scene ${sceneNum}: Z-Image ${state}${progress ? ` (${progress}%)` : ''}...`);
            results.push({ scene_number: sceneNum, status: 'processing', progress });
            continue;
          }
        }
