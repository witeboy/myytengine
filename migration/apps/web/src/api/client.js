...
//
// Gone from the old client: the KNOWN_FLAT set, the `resolvedShape` cache and the
// 404-retry fallback. There is exactly one path per function now.
...
const functions = {
  /**
   * Returns `{ data }` — the same envelope the Base44 SDK returned, which every
   * call site already destructures (`res.data || res`).
   *
   * Tolerates a trailing '/entry' in the name so any straggler call site written
   * against the old nested paths keeps working during the migration.
   */
  invoke: async (name, payload = {}, opts = {}) => {
    const clean = String(name).replace(/\/entry(\/entry)*$/, '');
    return request(`/api/fn/${clean}`, { body: payload, timeoutMs: opts.timeoutMs });
  },
};
